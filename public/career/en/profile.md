...
**Projects:**
- **Invoice API — Medical Billing System** (2025-12 – 2026-02): Complete billing system for a medical practice with multi-tenant support (physician, IT services, pharmacy). Draft→Finalized→Cancelled workflow with database-driven state machine (PL/pgSQL triggers). ZUGFeRD/EN16931-compliant invoice generation (PDF with embedded CII-XML). OpenEMR integration (patients, diagnoses, treatments). Server-side rendering with Jinja2 + HTMX. Snapshot pattern for audit-proof invoices. ~4,200 LOC Python, ~1,700 LOC SQL, 30+ endpoints, 13 DB migrations.
- **Intelligent Document Recognition & Automated Bank Reconciliation** (2025-08 – present): AI-powered pipeline for automatic recognition, classification and matching of business documents (invoices, receipts, contracts) to bank transactions. Multi-model OCR pipeline (PaddleOCR, Qwen-VL, Mistral-Nemo/Qwen3.5) for structured metadata extraction from scanned documents. Automatic document separation in multi-document PDFs using LLM feature extraction combined with trained ML model (GradientBoosting, F1=0.59). Index-based bank matching engine processing >3,000 transactions and >4,000 documents in ~7 seconds. Web UI (FastAPI) with document viewer, 4-column extraction comparison, bank account overview and configurable scoring parameters. DB-driven prompt versioning with loop-based retry and automatic fallback. Infrastructure: Docker Compose, PostgreSQL, Ollama LLM server with Tesla V100 and RTX GPUs.
- **Concept for Performance Documentation in Pain Therapy** (2026-02 – 2026-02): Concept for performance documentation of a pain therapy department with interdisciplinary input and reconciliation with OPS catalogue (German procedure classification).
...
- **Back Office Support and Process Improvement (Praxis für Schmerztherapie & TCM, Baden-Baden)** (2018-05 – 2025-05): Ongoing support for a private pain therapy & TCM practice: development of anaesthesia protocols, website setup and maintenance, digitalization and optimization of practice processes.
...
## Skills
- **Operating Systems:** Windows (bis 11) (proficient), Linux (basic)
...
---
Last updated: 2026-03-17 14:08:01
